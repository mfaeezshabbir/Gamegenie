...
function GameCard({ game }) {
  ...
  return (
    ...
    <Link href={`/games/${game.slug}`} <div className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex flex-wrap bg-gray-200 m-2 p-2 justify-center">
            {game.genres.map((genre, index) => (
              <span key={index} className="inline-block text-xs text-gray-700 font-roboto">{genre}</span>
            ))}
          </div>
          <div className="flex flex-wrap bg-gray-200 m-2 p-2 justify-center">
            {game.platforms.map((platform, index) => (
              <span key={index} className="inline-block text-xs text-gray-700 font-roboto">{platform}</span>
            ))}
          </div>
        </div>
