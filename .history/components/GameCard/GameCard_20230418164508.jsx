...
function GameCard({ game }) {
  ...
  {game.platforms.slice(0, 1).map((platform, index) => (
    <span key={index} className="inline-block text-xs text-gray-700 font-roboto px-1">{platform.name}</span>
  ))}
  {game.platforms.length > 1 && <span className="inline-block text-xs text-gray-700 font-roboto px-1">-</span>}
</div>

            <div className="flex flex-wrap bg-gray-200 m-2 p-2 justify-center">
              {game.platforms.map((platform, index) => (
                <span key={index} className="inline-block text-xs text-gray-700 font-roboto px-1">{platform}</span>
              ))}
            </div>
          </div>
