    ...
    <div className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex flex-wrap bg-gray-200 rounded-full ">
            {game.genres.map((genre, index) => (
              <span key={index} className="inline-block mr-2 mb-2 px-2 py-1 text-xs text-gray-700 font-roboto">{genre}</span>
            ))}
          </div>
          <div className="flex flex-wrap bg-gray-200 rounded-full ">
            {game.platforms.map((platform, index) => (
              <span key={index} className="inline-block mr-2 mb-2 px-2 py-1 text-xs text-gray-700 font-roboto">{platform}</span>
            ))}
          </div>
        </div>
